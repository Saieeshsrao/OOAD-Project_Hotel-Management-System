...
public class AddDishes extends JFrame {
	...
	public void addDishes() {
		...
		try {
			...
			if(ps.executeUpdate() > 0) {
				...
				Object[] newRow = {d1.getText(), d3.getText(), d2.getText()};
				model.addRow(newRow);
			}
		}
		catch(Exception ex) {
	...
	public void updateDishes() {
		...
		try {
			...
			if(d1.getText().length() > 0) ps.setString(1,d1.getText());
			else ps.setString(1, model.getValueAt(i, 1).toString());
			if(d2.getText().length() > 0) ps.setString(2,d2.getText());
			else ps.setString(2, model.getValueAt(i, 3).toString());
			if(d3.getText().length() > 0) ps.setString(3,d3.getText());
			else ps.setString(3, model.getValueAt(i, 2).toString());
			ps.setString(4, no);
			
			if(ps.executeUpdate() > 0) {
				JOptionPane.showMessageDialog(null, " Dish Updated ");
				if(d1.getText().length() > 0) model.setValueAt(d1.getText(), i, 1);
				if(d2.getText().length() > 0) model.setValueAt(d2.getText(), i, 3);
				if(d3.getText().length() > 0) model.setValueAt(d3.getText(), i, 2);
			}
		}
		catch(Exception ex) {
			ex.printStackTrace();
